"""Post-outcome analysis and reconstruction for frozen Exp13 evidence."""
...
def _load_and_verify(root: Path) -> list[dict[str, object]]:
    ...
    if {path.stem for path in paths} != expected or len(paths) != 540:
        raise RuntimeError("Exp13 disposition identity mismatch")
    rows = []
    for path in paths:
...
def analyze(root: Path) -> dict[str, object]:
    ...
    report = {"schema_version": 1, "experiment_id": "exp13-direct-hierarchy-v3", "matrix_total": 540,
    ...
    svg = f'<svg xmlns="http://www.w3.org/2000/svg" width="500" height="320"><rect width="100%" height="100%" fill="white"/><text x="250" y="30" text-anchor="middle" font-size="18">Exp13 primary mission success</text><line x1="50" y1="260" x2="450" y2="260" stroke="black"/>{bars}</svg>'
