...
The run executed from isolated commit `4307f6eb30504976d8250d240cb2ae9b1c7026b4`, which contains exact safety repair `18d03fb698cef4c35b6e47cdfcd48044ae73c870`. Every bundle retains raw 500 Hz observations, actions, references, events, metric inputs, safety and disposition evidence, with source/configuration/scenario/replay hashes. Deterministic annotations retain working and nonworking P4 examples; unavailable legacy-working and P6-nonworking classes remain explicitly `CLASS_NOT_OBSERVED` with denominator 96.

Three earlier v2 execution roots are retained only as invalid provenance. Attempts 1 and 3 stopped after all 96 tuning cells because source-ledger drift was detected; attempt 2 was operator-aborted after 20 tuning cells during a concurrent edit window. None produced or reached selection, evaluation, or fault-probe cells, and none affected any reported outcome. `ENGINEERING_P4_RESCUE_SUPERSESSION.json` and `ENGINEERING_P4_RESCUE_V2_EVIDENCE.json` enumerate all three roots with their exact disposition, completed cell/domain counts, file/byte totals, immutable marker hashes, and independently recomputed retained-tree inventory hashes. The historical markers' inventory claims do not match the retained trees and are explicitly marked false rather than silently repaired; attempt 1's nonresolving commit typo is preserved as a marker identifier and corrected by the exact repair commit above.
