"""Portable, independently reconstructable compact evidence for Exp16."""
...
EXPECTED_FREEZE_SHA256 = "66efee87fc163d45412e05b6474a38704f56f476a4cd894b1815972769e27097"
EXPECTED_RAW_INVENTORY_SHA256 = "1daf3b5e3747c80c2587d2b6b98e32ba81461395784e91799a6a7af468655acf"
BOOTSTRAP_SEED = 1313
...
def _sensitivity_values(rows: list[dict[str, object]], metric: str) -> dict[int, list[float]]:
    ...
    values = {}
    for seed in range(20262401, 20262406):
        cluster = []
        for family in sorted({str(row["family"]) for row in complete}):
            for severity in ("LOW", "HIGH"):
...
def _registered_effects(rows: list[dict[str, object]]) -> tuple[dict, dict, dict, dict]:
    ...
    expected_sensitivity_seeds = list(range(20262401, 20262406))
...
def reconstruct_derived(pack: Path, destination: Path) -> None:
    ...
    markdown = f"# Exp16 typed route-boundary replication\n\n**Formal disposition: {report['formal_disposition']}. No causal-lowest claim.**\n\nThis preregistered replication reused the reviewed Exp15 V2 contract with a fresh 202624 seed namespace and the typed route-unavailable boundary, without outcome-driven tuning. The portable pack contains all 540 dispositions and reconstructs every derived byte. {report['portability_limit']}\n"
