...
def _module():
    return importlib.import_module("experiments.16_route_boundary_replication.src.experiment")
...
def test_frozen_matrix_is_exactly_480_primary_plus_60_sensitivity() -> None:
    ...
    assert sorted({item.seed for item in specs if item.matrix_role == "PRIMARY"}) == list(range(20262401, 20262411))
    assert sorted({item.seed for item in specs if item.matrix_role == "SENSITIVITY"}) == list(range(20262401, 20262406))


def test_matrix_configuration_and_registered_doses_match_exp15_except_namespace() -> None:
    experiment = _module()
    reference = importlib.import_module("experiments.15_direct_hierarchy_replication.src.experiment")
    exp16 = experiment.matrix_specs()
    exp15 = reference.matrix_specs()
    ...
    assert [normalized(item) for item in exp16] == [normalized(item) for item in exp15]
    exp16_configuration = dict(experiment.frozen_configuration())
    exp15_configuration = dict(reference.frozen_configuration())
    assert exp16_configuration.pop("runtime_route_unavailable") == (
        "SAFE_ABORT_COMPLETE_SCORED_FAILURE"
    )
    for key in ("experiment_id", "primary_seeds", "sensitivity_seeds"):
        exp16_configuration.pop(key)
        exp15_configuration.pop(key)
    assert exp16_configuration == exp15_configuration
    for left, right in zip(exp16, exp15, strict=True):
...
def test_runtime_route_exhaustion_is_complete_scored_failure_not_invalid(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    experiment = _module()
    runtime = importlib.import_module("experiments.03_recovery.src.v3_runtime")
    spec = experiment.DirectSpec(
        experiment.Architecture.R3,
        "motion-path-infeasible",
        "HIGH",
        20262401,
        experiment.PRIMARY_CONTROLLER_ID,
        "PRIMARY",
    )
    realization, receipt = experiment._cell_precheck(spec)
    assert receipt.disposition == "READY"

    class RuntimeRouteUnavailable(Exception):
        pass

    monkeypatch.setattr(runtime, "MotionPlanUnavailable", RuntimeRouteUnavailable)
    monkeypatch.setattr(
        runtime,
        "_waypoint_path",
        lambda *_args: (_ for _ in ()).throw(
            RuntimeRouteUnavailable("runtime route unavailable")
        ),
    )
    monkeypatch.setattr(experiment, "matrix_specs", lambda: (spec,))
    monkeypatch.setattr(experiment, "_cell_precheck", lambda _spec: (realization, receipt))
    monkeypatch.setattr(
        experiment,
        "_validate_freeze",
        lambda _output: {"source_commit": "a" * 40},
    )
    monkeypatch.setattr(experiment, "_validate_preflight", lambda _output: {})
    monkeypatch.setattr(experiment, "_validate_first50_release", lambda _output: None)

    root = tmp_path / experiment.EXPERIMENT_ID
    (root / "preflight/receipts").mkdir(parents=True)
    (root / "freeze.json").write_bytes(b"{}\n")
    preflight = {
        "schema_version": 1,
        "episode_id": spec.episode_id,
        "disposition": receipt.disposition,
        "parameter_sha256": realization.parameter_sha256,
        "receipt": experiment._receipt_payload(receipt),
    }
    (root / "preflight/receipts" / f"{spec.episode_id}.json").write_bytes(
        experiment.canonical_bytes(preflight)
    )

    result = experiment.execute(root, limit=1)
    row = json.loads(
        (root / "raw/dispositions" / f"{spec.episode_id}.json").read_text()
    )
    assert result["completed"] == 1
    assert row["disposition"] == "COMPLETE"
    assert row["terminal"] == "FAILURE"
    assert row["mission_success"] is False
    assert row["aborts"] == 1
    assert "execution_stage" not in row


def test_realization_is_paired_and_registered_dose_is_used() -> None:
    experiment = _module()
    specs = experiment.matrix_specs()
    cells = [item for item in specs if item.family == "control-dropout" and item.severity == "HIGH" and item.seed == 20262401]
...
def test_exp16_schema_contains_no_causal_oracle_fields() -> None:
    experiment = _module()
