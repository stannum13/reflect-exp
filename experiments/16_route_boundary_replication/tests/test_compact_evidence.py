...
def _module():
    return importlib.import_module("experiments.16_route_boundary_replication.src.compact_evidence")
...
def test_sensitivity_plan_and_actual_n_eff_are_exactly_preregistered() -> None:
    module = _module()
    seeds = list(range(20262401, 20262406))
    ...
    effect, draws = module._effect({20262401: [1.0], 20262403: [0.0]}, np.asarray([[20262401, 20262403]] * 10_000))
...
def test_reconstruction_effect_path_reports_actual_missing_clusters_without_keyerror() -> None:
    module = _module()
    rows = []
    for seed in range(20262401, 20262410):
    ...
    for seed in range(20262401, 20262405):
        rows.append({
    ...
    assert set(plans["R3-R2:mission_success"].ravel()) == set(range(20262401, 20262410))
...
def compact_pack() -> Path:
    module = _module()
    if module.EXPECTED_FREEZE_SHA256 == "UNSEALED_BEFORE_OUTCOME":
        pytest.skip("Exp16 outcome pack is intentionally unsealed before audit/freeze/run")
    return _repository_root() / "reports/evidence/exp16-route-boundary-replication-v1"
