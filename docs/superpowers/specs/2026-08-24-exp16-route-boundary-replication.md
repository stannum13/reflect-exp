# Exp16 typed route-boundary replication

**Status:** awaiting independent source/record audit; not approved to freeze or run
**Date:** 2026-08-24
**Predecessor:** Exp16 uses the independently reviewed Exp15 V2 lifecycle and
scientific contract without tuning. No Exp16 outcome exists or informed this
design.

Exp16 asks whether the bounded direct-outcome signal replicates after the
already-reviewed typed runtime route-unavailable boundary is made explicit. It
makes no learned-VLA, real-world-transfer, causal-minimality, or
lowest-sufficient-recovery claim. Observable-only policy, scorer, P6/P4
controllers, disturbance families/doses, 540-cell matrix, metrics, and
scientific gates remain exactly those independently reviewed for Exp15 V2.
Only the fresh identity, seed namespace, paths, and declared boundary below
change.

## Fresh identity and matrix

- Experiment: `exp16-route-boundary-replication-v1`; episode prefix `exp16v1`.
- P6 primary seeds: exactly `20262401..20262410`.
- P4 R3 sensitivity seeds: exactly `20262401..20262405`.
...
## Preregistered typed route-unavailable boundary

The production runtime's typed `MotionPlanUnavailable` condition means that
neither frozen waypoint candidate can clear the active obstacle at the moment a
motion recovery begins. When this condition occurs during outcome execution,
the motion decision is replaced by exactly one `SAFE_ABORT` with reason
`MOTION_PLANNER_NO_ROUTE`; its budget remains unchanged and the executor holds
thereafter. The episode is not an infrastructure error. It must return through
the normal scorer as terminal `FAILURE`, be sealed with disposition `COMPLETE`,
retain its raw episode evidence, and contribute to every registered metric and
gate. Only unrelated exceptions remain eligible for `INVALID_EXECUTION`.

This boundary is frozen as
`runtime_route_unavailable=SAFE_ABORT_COMPLETE_SCORED_FAILURE`. It changes no
matrix cell, disturbance dose, controller, metric, resampling rule, effective
sample-size rule, threshold, disposition schema, or evidence-pack schema.
...
`product([20262401,...,20262405], repeat=5)`, containing 3,125 five-seed tuples.
