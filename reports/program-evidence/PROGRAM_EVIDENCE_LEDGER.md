...
The current evidence justifies a **typed, layered synthetic reference architecture**, not completion of the Reflect Lite research program. Five bounded workstreams are accepted at their narrow scientific or engineering scopes. Exp13 and Exp15 V2 are separately approved as authenticated portable archives whose formal dispositions remain `INVALID_EXPERIMENT`; their hierarchy results are directional, not confirmatory. Together the program supports keeping live belief separate from durable geometry/semantics, keeping episodic events separately addressable, separating appearance-sensitive semantics from geometry, and making refresh/retrigger/replan explicit policies. It does not yet establish a learned VLA policy, real perception, physical transfer, a valid confirmatory end-to-end three-level hierarchy, or that recovery should always occur at the lowest sufficient level.
...
| Motion refresh/retrigger/replan | **Expose separately from semantic replanning.** Geometry-aware motion may replan without changing mission semantics; route exhaustion is a motion-domain result that should safe-abort rather than silently escalate semantics. | Exp12 shows a geometry-aware motion path transfers across appearance; Exp10/11 distinguish failure families and trigger policies. Exp13 and Exp15 V2 supply independently reviewed portable directional packs. | Exp12 contains no post-plan recovery event and its hierarchy equals RGB-D motion. Exp13 and Exp15 remain formally invalid; Exp15 exposes the initial-pose-precheck versus live-pose-retry boundary that requires a fresh preregistered run. |
...
3. **Directional portable only:** Exp13 and Exp15 V2 are independently approved as reconstructable archives. Both remain formal `INVALID_EXPERIMENT`; Exp15 V2 closely repeats the P6 hierarchy direction but cannot confirm it because three P4 route exceptions invalidate the frozen matrix.
4. **Rejected:** Exp14 cannot qualify a mission-memory/recovery mechanism despite replayable descriptive raw facts.
5. **Not run:** pi0.5 semantic-plan inference and the external VLA lane remain open.
...
Therefore the program status is **PARTIAL_INDICATIVE_EVIDENCE — CONTINUE**, not `COMPLETE`. The next promotion gate is not more scaffolding: run one compact checkpoint-backed learned-policy/VLA study against an honestly supported output interface, and one fresh direct layer-recovery replication in which dynamic route exhaustion is preregistered as a safe-abort task failure. Only after those pass should the full architecture arrow diagram be called experimentally selected.
